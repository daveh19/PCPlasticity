...
int main( int argc, char *argv[] ){
	...
	gsl_multifit_function_fdf fdf; // function to fit
	...
	double x_init[8] = {185,80,0.07,0.6,0.2,0.522,2.3809e-4,7.9365e-5};
	...
	s = gsl_multifit_fdfsolver_alloc (T, 17, 8);
	...
	x = gsl_vector_view_array(x_init, 8); // setup initial guess

	status = gsl_multifit_fdfsolver_set (s, &fdf, &x.vector); // setup solver
